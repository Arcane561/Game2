...
{
public:
	std::vector<IntRect> frames, frames_flip; // массив "кадров" frames_flip отраженные кадры
...
public:
	...
	std::map<std::string, Animation> animList; // список анимации 
   ...
   //создание анимаций вручную
	void create(std::string name, Texture &texture, int x, int y, int w, int h, int count, float speed, int step=0, bool Loop=true)
	{
		...
		for (int i=0;i<count;i++) //Создаем массив "прмоугольников"
		{
